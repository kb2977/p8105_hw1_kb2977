...
My factor vector created above has `r length(prob2_df$fac_vec2)`, a mean of `r mean(prob2_df$x)`, a median of `median(prob2_df$x)`, and a proportion of cases `r mean(prob2_df$log_vec2)`.
...
The red points are the x and y values that do not meet the log_vec2 statement (FALSE) whereas the blue points are the x and y values that do meet the log_vec2 statement (TRUE).
...
```
The dark blue points represent the points when num_vec2 = 0 (FALSE) and the light blue point represent the values in which num_vec2 = 1 (TRUE).
...
```
The blue points represent when fac_vec2 = 1 and the red points represent when fac_vec2 = 0. This scale is different from the numeric scatterplot (sp_num_vec) because there are only a set of values that fac_vec can take (in this case, 0 or 1) whereas there are an infinite number of values that a numeric vector can take.

Exporting the first scatterplot (sp_log_vec) to my project directory.
#HOW DOES THIS KNOW TO DO THE FIRST SCATTER PLOT?
`ggsave("scatterplot_log_vec.pdf", height = 4, width = 6)`